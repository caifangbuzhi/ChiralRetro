...
class BeamSearch:
    ...
    def run_search(self, react_smi, prod_smi: str, max_steps: int = 8, rxn_class: int = None) -> List[dict]:
        product = Chem.MolFromSmiles(prod_smi)
        Chem.Kekulize(product)
        reactant = Chem.MolFromSmiles(react_smi)
        Chem.Kekulize(reactant)
        product, reactant = addH(product, reactant)   
