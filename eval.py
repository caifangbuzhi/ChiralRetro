...
from models import ChiralRetro, BeamSearch
lg = RDLogger.logger()
lg.setLevel(4)
...
def main():
    ...
    checkpoint = torch.load(os.path.join(exp_dir, f'{args.epochs}'))
    config = checkpoint['saveables']

    model = ChiralRetro(**config, device=DEVICE)
        ...
        for idx in p_bar:
            ...
            with torch.no_grad():
                top_k_results = beam_model.run_search(
                    r, prod_smi=p, max_steps=args.max_steps, rxn_class=rxn_class)
