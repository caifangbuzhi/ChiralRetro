...
ATOM_SYMBOL_LIST = ['H', 'C', 'N', 'O', 'S', 'F', 'Cl', 'Br', 'H', 'Si', 'P', 'B', 'I', 'Li', 'Na', 'K', 'Ca',
...
NUM_Hs = [0, 1, 2, 3, 4]
CIP_CHIRALITY = ['R', 'S', None]
CHIRALTAG_PARITY = [ChiralType.CHI_TETRAHEDRAL_CW, ChiralType.CHI_TETRAHEDRAL_CCW, ChiralType.CHI_UNSPECIFIED, ChiralType.CHI_OTHER]
...
ATOM_FDIM = len(ATOM_SYMBOL_LIST) + len(DEGREES) + len(FORMAL_CHARGE) + \
    len(VALENCE) + len(NUM_Hs) + len(CHIRALTAG_PARITY) + len(HYBRIDIZATION) + 1
...
def get_atom_features(atom: Chem.Atom, rxn_class: int = None, use_rxn_class: bool = False) -> List[Union[bool, int, float]]:
    ...
    if use_rxn_class:    
        atom_features = one_of_k_encoding(atom.GetSymbol(), ATOM_SYMBOL_LIST) + \
            ...
            one_of_k_encoding(atom.GetChiralTag(), CHIRALTAG_PARITY) + \
