...
class RetroEditDataset(Dataset):
    def __init__(self, data_dir: str, **kwargs):
        ...
        self.data_files = [
            os.path.join(self.data_dir, 'feat', file)
            for file in os.listdir(os.path.join(self.data_dir, 'feat'))
            if "batch-" in file
        ]
        self.chiral_data_files = [
            os.path.join(self.data_dir, "chiral", file)
            for file in os.listdir(os.path.join(self.data_dir, "chiral"))
            if "batch-" in file]
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor]:
        ...
        chiral_batch_tensors = torch.load(self.chiral_data_files[idx], map_location='cpu')

        return batch_tensors, chiral_batch_tensors
    ...
    def collate(self, attributes: List[Tuple[torch.tensor]]) -> Tuple[torch.Tensor]:
        ...
        attributes1 = attributes[0][0]
        attributes2 = attributes[0][1]
        graph_seq_tensors, edit_seq_labels, seq_mask = attributes1
        U_list, Z_list, D_list, N_list = attributes2
...
class RetroEvalDataset(Dataset):
    ...
    def collate(self, attributes: List[ReactionData]) -> Tuple[str, List[Tuple], List[List], Optional[List[int]]]:
        ...
        react_smi = [rxn_data.rxn_smi.split(">>")[0]
        ...
        if self.use_rxn_class:
            ...
            return react_smi, prod_smi, edits, edits_atom, rxn_classes
        else:
            return react_smi, prod_smi, edits, edits_atom, None
