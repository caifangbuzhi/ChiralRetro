...
from models import ChiralRetro
...
def to_device(tensors: Union[List, torch.Tensor]) -> Union[List, torch.Tensor]:
...
def train_epoch(args, epoch, model, train_data, loss_fn, edit_loss1, edit_loss2, optimizer):   
    torch.cuda.empty_cache()
    model.train()
    train_loss = 0
    train_acc = 0
    for batch_id, batch_data in enumerate(train_data):
        graph_seq_tensors, seq_labels, seq_mask, U_list, Z_list, D_list, N_list = batch_data
        ...
        for idx in range(max_seq_len):
            if idx == 0:
                U_list = to_device(U_list)
                Z_list = to_device(Z_list)
                D_list = to_device(D_list)
                N_list = to_device(N_list)
                edit_labels_idx = model.to_device(seq_labels[idx])
                loss_batch  = []
                
                for i in range(batch_size):
                    numbond = graph_seq_tensors[0][1][1][i][1]
                    idxinbond = numbond * 6
                    probabilities = F.softmax(seq_edit_scores[0][i],dim=0)
                    edit_loss = edit_loss2(seq_edit_scores[0][i].unsqueeze(0),torch.argmax(edit_labels_idx[i]).unsqueeze(0).long()).sum()
                    edit_labels_idx[i] = edit_labels_idx[i].to(torch.float32)
                    N = int(N_list[i].item())
                    
                    equal_loss = 1.0/math.factorial(N) * torch.dot((edit_labels_idx[i] * probabilities), U_list[i])
                    distance_loss = N * 1.0/math.factorial(N) * 1/numbond * torch.dot((Z_list[i] + torch.cat((1/D_list[i][:idxinbond],D_list[i][idxinbond:]),dim=0)), (edit_labels_idx[i] * probabilities))
                    loss_batch.append(edit_loss/2+equal_loss/4+distance_loss/4)
...
def test(model, valid_data):
    ...
    with torch.no_grad():
        for batch_id, batch_data in enumerate(valid_data):
            react_smibatch, prod_smi_batch, edits_batch, edits_atom_batch, rxn_classes = batch_data
            for idx, prod_smi in enumerate(prod_smi_batch):
                if rxn_classes is None:
                    edits, edits_atom = model.predict(react_smibatch[idx], prod_smi)
                else:
                    edits, edits_atom = model.predict(
                        react_smibatch[idx], prod_smi, rxn_class=rxn_classes[idx])
...
def main(args):
    ...
    model = ChiralRetro(config=model_config, atom_vocab=atom_vocab,
    ...
    edit_loss1 = nn.BCEWithLogitsLoss(reduction='none')
    edit_loss2 = nn.CrossEntropyLoss(reduction='none')
    ...
    for epoch in range(args['epochs']):
        train_loss, train_acc = train_epoch(
            args, epoch, model, train_data, loss_fn, edit_loss1, edit_loss2, optimizer)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--print_every', type=int,
                        default=100, help='Print during train process')
    parser.add_argument('--num_workers', default=6,
                        help='Number of processes for data loading')
    parser.add_argument("--encoder", default='DMPNN')
